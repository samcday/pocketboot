...
pub(crate) struct Partition {
    ...
    pub(crate) partname: Option<String>,
    pub(crate) dev_path: PathBuf,
    pub(crate) devnum: String,
...
impl Partition {
    ...
    pub(crate) fn fastboot_name(&self) -> &str {
        self.name()
    }
...
pub(crate) fn find(name: &str) -> io::Result<Partition> {
    Resolver::new(PathBuf::from(SYS_BLOCK), PathBuf::from(DEV)).find(name)
}

#[cfg_attr(not(feature = "blob-wrangler"), allow(dead_code))]
pub(crate) fn find_optional(name: &str) -> io::Result<Option<Partition>> {
    match find(name) {
...
pub(crate) fn list() -> io::Result<Vec<Partition>> {
...
mod tests {
    ...
    fn resolves_partition_by_partname() {
        ...
        assert_eq!(partition.devnum, "179:1");
    }
